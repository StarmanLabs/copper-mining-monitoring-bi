"""Deterministic valuation engine for the advanced appendix.

This module is intentionally secondary to the monthly monitoring platform.
It provides a reusable, workbook-seeded valuation layer for deterministic
scenario comparison and downside framing, but it does not claim asset-grade
fiscal realism or full workbook parity.
"""
...
class ScenarioParameters:
    """Explicit deterministic valuation inputs used by the appendix layer."""
    ...
    def cash_tax_proxy_rate(self) -> float:
        """Return the workbook-aligned cash tax proxy used by the appendix.

        Royalties and the special levy remain visible in the parameter set for
        transparency, but the current public-safe appendix does not attempt to
        recreate a full fiscal regime. The cash-flow chain therefore keeps the
        simpler income-tax proxy that is aligned with the workbook benchmark
        path the repository is comparing against.
        """
    ...
    def base_site_cost_usd_per_tonne(self) -> float:
        """Return the base operating cost used by the deterministic base case."""
...
def _as_scalar_factor(value: float | np.ndarray, parameter_name: str) -> float:
    """Return a scalar appendix factor and fail clearly for vector misuse."""

    scalar = np.asarray(value, dtype=float)
    if scalar.ndim == 0:
        return float(scalar)
    raise ValueError(
        f"{parameter_name} must be a scalar in the deterministic appendix model; "
...
def _annual_or_scalar_vector(
    annual_inputs: pd.DataFrame,
    column_name: str,
    fallback_value: float,
) -> np.ndarray:
    if column_name in annual_inputs.columns:
        return annual_inputs[column_name].to_numpy(dtype=float)
    return np.full(len(annual_inputs), float(fallback_value), dtype=float)


def _annual_capex_schedule(
    annual_inputs: pd.DataFrame,
    params: ScenarioParameters,
    capex_factor: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if {"initial_capex_usd", "sustaining_capex_usd"}.issubset(annual_inputs.columns):
        initial_capex = annual_inputs["initial_capex_usd"].to_numpy(dtype=float)
        sustaining_capex = annual_inputs["sustaining_capex_usd"].to_numpy(dtype=float)
    else:
        years = annual_inputs["year"]
...
def _build_operating_profile(
    ...
    unit_cost_vector = _as_year_vector(unit_cost_usd_per_tonne, size)
    ...
    df["unit_opex_usd_per_tonne"] = unit_cost_vector * opex_vector
...
def build_expansion_profile(
    ...
    """Return the deterministic expanded-operation profile used in the appendix.

    The throughput uplift is intentionally stylized through a logistic ramp. If
    annual unit-cost and capex columns are present in the canonical input table,
    they take precedence over the scalar fallback parameters.
    """
    ...
    capex_multiplier = _as_scalar_factor(capex_factor, "capex_factor")
    logistic_term = 1.0 / (1.0 + np.exp(-params.logistic_steepness * (years - params.logistic_midpoint)))
    expanded_tonnes = (
        df["base_processed_tonnes"].to_numpy(dtype=float)
        * throughput_vector
        * (1.0 + params.uplift_pct * logistic_term)
    )
    ...
    df = _build_operating_profile(
        ...
        unit_cost_usd_per_tonne=_annual_or_scalar_vector(
            ...
            params.expansion_unit_cost_usd_per_tonne,
        ),
        price_factor=price_factor,
        grade_factor=grade_factor,
        recovery_factor=recovery_factor,
    ...
    initial_capex, sustaining_capex, total_capex = _annual_capex_schedule(annual_inputs, params, capex_multiplier)
    ...
    df["discounted_fcf_usd"] = df["free_cash_flow_usd"] * df["discount_factor"]
    df.attrs["initial_capex_year0_usd"] = _initial_capex_year0(params, capex_multiplier)
    return df
...
def build_incremental_expansion_profile(
    ...
    """Return the incremental expansion profile used for workbook comparison.

    The working-capital treatment is kept deliberately simple and benchmark-
    oriented: the incremental revenue balance is invested through the project
    life and released in the final year. This is useful for transparent
    comparison, but it should not be described as a full asset-grade treasury
    model.
    """
...
def irr_from_profile(profile: pd.DataFrame) -> float:
    """Return the project IRR using explicit year-0-aware cash flows."""
